...
const USER_ID = "nisa"; // single-user app
...
export default function BudgetApp() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (c.data && c.data.length > 0) setCategories(c.data);
        ...
        await supabase.from("categories").upsert(DEFAULT_CATS.map(cat => ({ ...cat, user_id: USER_ID })));
      }
  ...
  const dbInsert = async (table, row) => {
    ...
    const { error } = await supabase.from(table).insert({ ...row, user_id: USER_ID });
    setSyncStatus(error ? "error" : "synced");
    return !error;
  };
  const dbUpdate = async (table, id, row) => {
    setSyncStatus("syncing");
    const { error } = await supabase.from(table).update(row).eq("id", id).eq("user_id", USER_ID);
    setSyncStatus(error ? "error" : "synced");
    return !error;
  };
  ...
  const dbUpsert = async (table, row) => {
    setSyncStatus("syncing");
    const { error } = await supabase.from(table).upsert({ ...row, user_id: USER_ID });
    setSyncStatus(error ? "error" : "synced");
    return !error;
  };
  ...
  const expByCat = categories.map(c=>({
    ...
    value: expenses.filter(e=>e.category===c.id).reduce((s,e)=>s+Number(e.amount),0)
  })).filter(c=>c.value>0);

  const nwHistory = (()=>{
    const ms=[...new Set([...expenses,...savings,...income,...investments].map(e=>e.date).filter(Boolean).map(d=>d.slice(0,7)))].sort();
    return ms.map(m=>{
      const e=expenses.filter(x=>moOf(x.date)<=m).reduce((s,x)=>s+Number(x.amount),0);
      const s=savings.filter(x=>moOf(x.date)<=m).reduce((s,x)=>s+Number(x.amount),0);
      const i=income.filter(x=>moOf(x.date)<=m).reduce((s,x)=>s+Number(x.amount),0);
      const inv=investments.filter(x=>moOf(x.date)<=m).reduce((s,x)=>s+Number(x.amount),0);
      return { label:MONTHS[parseInt(m.split("-")[1])-1], v:s+i+inv-e };
    });
  })();
  ...
  const handleFile = async e => {
    ...
    reader.onload=async ev=>{
      ...
      } catch { setScanMsg("Could not read receipt — fill in manually"); }
      setScanning(false);
    };
  ...
  const saveExpense = async () => {
    ...
    if (editId) {
      await dbUpdate("expenses", editId, entry);
      setExpenses(expenses.map(x=>x.id===editId?entry:x));
    } else {
      await dbInsert("expenses", entry);
  ...
  const saveSimple = async (table, list, setList) => {
    ...
    const entry={ id:editId||Date.now(), ...form, date:form.date||toDay() };
    if (editId) {
      await dbUpdate(table, editId, entry);
      setList(list.map(x=>x.id===editId?entry:x));
    } else {
      await dbInsert(table, entry);
      setList([entry,...list]);
    }
    closeModal();
  };
  ...
  const saveGoal = async () => {
    ...
    if (editId) {
      await dbUpdate("goals", editId, entry);
      setGoals(goals.map(x=>x.id===editId?entry:x));
    } else {
      await dbInsert("goals", entry);
  ...
  const saveCat = async () => {
    ...
    if (editId) {
      const updated={ ...categories.find(c=>c.id===editId), ...form };
      await dbUpdate("categories", editId, updated);
      setCategories(categories.map(c=>c.id===editId?updated:c));
    } else {
      const entry={ id:Date.now().toString(), budget:"", ...form };
      await dbUpsert("categories", entry);
      setCategories([...categories, entry]);
    }
    closeModal();
  };

  const deleteRow = async (table, id, list, setList) => {
  ...
  const topup = async g => {
    ...
    if (!isNaN(add)&&add>0) {
      const updated={ ...g, saved:Number(g.saved)+add };
      await dbUpdate("goals", g.id, updated);
    ...
    <div style={{ minHeight:"100vh", background:T.bg, color:T.text, fontFamily:"'DM Sans',sans-serif", paddingBottom:100 }}>
      ...
      <div style={{ maxWidth:520, margin:"0 auto", padding:"20px 16px" }}>
        ...
        {tab==="Dashboard" && (
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"flex",gap:12}}>
              {[{l:"Income",val:totInc},{l:"Saved",val:totSav},{l:"Invested",val:totInv}].map(s=>(
                <Card key={s.l} style={{flex:1,padding:"14px 16px"}}><Lbl>{s.l}</Lbl><div style={{fontSize:16,fontFamily:"'Instrument Serif',serif"}}>{sgd(s.val)}</div></Card>
              ))}
            </div>
            <Card>
              ...
              <div style={{display:"flex",alignItems:"center",gap:20,marginTop:10}}>
                ...
                <div style={{flex:1,display:"flex",flexDirection:"column",gap:10}}>
                  {expByCat.length ? expByCat.map(c=>(
                    <div key={c.label}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                        <div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:8,height:8,borderRadius:"50%",background:c.color,flexShrink:0}}/><span style={{fontSize:13}}>{c.label}</span></div>
                        <span style={{fontSize:13,fontWeight:700}}>{sgd(c.value)}</span>
                      </div>
                      {(()=>{ const cat=categories.find(x=>x.label===c.label); return cat?.budget?<Bar value={c.value} max={Number(cat.budget)} showPct/>:null; })()}
                    </div>
                  )) : <span style={{fontSize:13,color:T.muted}}>No expenses yet</span>}
                </div>
              </div>
            </Card>
              ...
              {expenses.slice(0,5).map(e=>(
                <div key={e.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.border}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:34,height:34,borderRadius:10,background:getCat(e.category).color+"12",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{width:8,height:8,borderRadius:"50%",background:getCat(e.category).color}}/></div>
                    <div><div style={{fontSize:13,fontWeight:600}}>{e.merchant||getCat(e.category).label}</div><div style={{fontSize:11,color:T.muted}}>{e.date}</div></div>
                  </div>
                  <span style={{fontSize:14,fontWeight:700}}>−{sgd(e.amount)}</span>
                </div>
              ))}
              {!expenses.length&&<div style={{fontSize:13,color:T.muted,paddingTop:8}}>No expenses yet</div>}
            </Card>
          </div>
        )}
        ...
        {tab==="Expenses" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:22,fontFamily:"'Instrument Serif',serif"}}>Expenses</div>
              <Btn onClick={()=>{setEditId(null);openModal("expense");}}>+ Add</Btn>
            </div>
            ...
            {expenses.map(e=>(
              <div key={e.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"13px 0",borderBottom:`1px solid ${T.border}`}}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <div style={{width:38,height:38,borderRadius:12,background:getCat(e.category).color+"12",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><div style={{width:9,height:9,borderRadius:"50%",background:getCat(e.category).color}}/></div>
                  <div>
                    <div style={{fontSize:14,fontWeight:600}}>{e.merchant||getCat(e.category).label}</div>
                    <div style={{display:"flex",gap:6,marginTop:3,alignItems:"center"}}><Pill label={getCat(e.category).label} color={getCat(e.category).color}/><span style={{fontSize:11,color:T.muted}}>{e.date}</span></div>
                    {e.note&&<div style={{fontSize:11,color:T.muted,marginTop:2}}>{e.note}</div>}
                  </div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
                  <span style={{fontSize:15,fontWeight:700}}>−{sgd(e.amount)}</span>
                  <button onClick={()=>{setEditId(e.id);openModal("expense",{...e});}} style={{background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:13,padding:"2px 4px"}}>✎</button>
                  <button onClick={()=>deleteRow("expenses",e.id,expenses,setExpenses)} style={{background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:18,padding:"2px 4px"}}>×</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {tab==="Savings" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:22,fontFamily:"'Instrument Serif',serif"}}>Savings</div>
              <div style={{display:"flex",gap:8}}><Btn variant="outline" size="sm" onClick={()=>{setEditId(null);openModal("income");}}>+ Income</Btn><Btn size="sm" onClick={()=>{setEditId(null);openModal("savings");}}>+ Savings</Btn></div>
            </div>
            <div style={{display:"flex",gap:12,marginBottom:20}}>
              <Card style={{flex:1}}><Lbl>Total Saved</Lbl><div style={{fontSize:20,fontFamily:"'Instrument Serif',serif"}}>{sgd(totSav)}</div></Card>
              <Card style={{flex:1}}><Lbl>Total Income</Lbl><div style={{fontSize:20,fontFamily:"'Instrument Serif',serif"}}>{sgd(totInc)}</div></Card>
            </div>
            {[...savings.map(s=>({...s,_t:"Savings"})),...income.map(s=>({...s,_t:"Income"}))].sort((a,b)=>(b.date||"").localeCompare(a.date||"")).map(e=>(
              <div key={e.id+e._t} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"13px 0",borderBottom:`1px solid ${T.border}`}}>
                <div><div style={{fontSize:14,fontWeight:600}}>{e.note||e._t}</div><div style={{display:"flex",gap:6,marginTop:3}}><Pill label={e._t} color={T.accent}/><span style={{fontSize:11,color:T.muted}}>{e.date}</span></div></div>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:15,fontWeight:700,color:T.success}}>+{sgd(e.amount)}</span>
                  <button onClick={()=>{ if(e._t==="Savings") deleteRow("savings",e.id,savings,setSavings); else deleteRow("income",e.id,income,setIncome); }} style={{background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:18}}>×</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {tab==="Net Worth" && (
          <div>
            ...
            <Card style={{textAlign:"center",marginBottom:14,padding:"28px 20px"}}>
              <Lbl>Total Net Worth</Lbl>
              <div style={{fontSize:44,fontFamily:"'Instrument Serif',serif",color:netWorth<0?T.danger:T.text,letterSpacing:"-0.02em",marginTop:6}}>{sgd(netWorth)}</div>
              <div style={{fontSize:12,color:T.muted,marginTop:8}}>Savings + Income + Investments − Expenses</div>
            </Card>
            <Card style={{marginBottom:14}}><Lbl>Net Worth History</Lbl><div style={{marginTop:10}}><LineChart points={nwHistory}/></div></Card>
            {[{label:"Income",val:totInc,action:()=>openModal("income"),add:"+ Income",neg:false},{label:"Savings",val:totSav,action:()=>openModal("savings"),add:"+ Savings",neg:false},{label:"Investments",val:totInv,action:()=>openModal("investment"),add:"+ Investment",neg:false},{label:"Expenses",val:totExp,action:()=>openModal("expense"),add:"+ Expense",neg:true}].map(r=>(
              <Card key={r.label} style={{marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><Lbl>{r.label}</Lbl><div style={{fontSize:20,fontFamily:"'Instrument Serif',serif",color:r.neg?T.danger:T.text}}>{r.neg?"−":""}{sgd(r.val)}</div></div>
                <Btn variant="outline" size="sm" onClick={r.action}>{r.add}</Btn>
              </Card>
            ))}
          </div>
        )}
        ...
        {tab==="Goals" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:22,fontFamily:"'Instrument Serif',serif"}}>Money Goals</div>
              <Btn onClick={()=>{setEditId(null);openModal("goal");}}>+ Goal</Btn>
            </div>
            ...
            {goals.map(g=>{ const pct=Math.min((Number(g.saved)/Number(g.target))*100,100); return (
              <Card key={g.id} style={{marginBottom:14}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  ...
                  <div style={{display:"flex",gap:6}}><button onClick={()=>{setEditId(g.id);openModal("goal",{...g});}} style={{background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:13}}>✎</button><button onClick={()=>deleteRow("goals",g.id,goals,setGoals)} style={{background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:18}}>×</button></div>
                </div>
                {g.note&&<div style={{fontSize:12,color:T.muted,margin:"4px 0 10px"}}>{g.note}</div>}
                <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:6}}><span style={{fontWeight:600,color:T.success}}>{sgd(g.saved)} saved</span><span style={{color:T.muted}}>of {sgd(g.target)}</span></div>
                <Bar value={Number(g.saved)} max={Number(g.target)} showPct/>
                <div style={{marginTop:12}}><Btn variant="outline" size="sm" onClick={()=>topup(g)}>+ Top up</Btn></div>
              </Card>
            );})}
          </div>
        )}
        ...
        {tab==="Settings" && (
          <div>
            ...
            <Card style={{marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><Lbl>Expense Categories</Lbl><Btn size="sm" onClick={()=>{setEditId(null);openModal("cat");}}>+ New</Btn></div>
              {categories.map(c=>(
                <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.border}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:12,height:12,borderRadius:"50%",background:c.color,border:`1.5px solid ${T.border2}`,flexShrink:0}}/><div><div style={{fontSize:14,fontWeight:600}}>{c.label}</div>{c.budget&&<div style={{fontSize:11,color:T.muted}}>Budget: {sgd(c.budget)}/mo</div>}</div></div>
                  <div style={{display:"flex",gap:10}}><button onClick={()=>{setEditId(c.id);openModal("cat",{...c});}} style={{background:"none",border:"none",cursor:"pointer",color:T.sub,fontSize:13}}>✎ Edit</button><button onClick={()=>deleteRow("categories",c.id,categories,setCategories)} style={{background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:13}}>✕</button></div>
                </div>
              ))}
            </Card>
            <Card style={{marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><Lbl>Investments</Lbl><Btn size="sm" onClick={()=>{setEditId(null);openModal("investment");}}>+ Add</Btn></div>
              {!investments.length&&<div style={{fontSize:13,color:T.muted}}>No investments tracked yet.</div>}
              {investments.map(e=>(<div key={e.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.border}`}}><div><div style={{fontSize:14,fontWeight:600}}>{e.note||"Investment"}</div><div style={{fontSize:11,color:T.muted}}>{e.date}</div></div><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14,fontWeight:700}}>{sgd(e.amount)}</span><button onClick={()=>deleteRow("investments",e.id,investments,setInvestments)} style={{background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:18}}>×</button></div></div>))}
            </Card>
            <Card style={{marginBottom:14}}>
              <Lbl>Sync</Lbl>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <SyncDot status={syncStatus}/>
                <Btn variant="outline" size="sm" onClick={loadAll}>↻ Refresh</Btn>
              </div>
            </Card>
